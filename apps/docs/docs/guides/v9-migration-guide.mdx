...
slug: /guides/v9-migration-guide
hide_title: true
---
...
<VStack gap={5}>
<ContentHeader title="v9 Migration Guide" />
...
- 🌬️ Migration off the unmaintained `react-spring` library begins (Stepper, SlideButton)
...
If you experience any trouble migrating we're ready to help — reach out via Slack.
...
- `@coinbase/cds-web-visualization@4.0.0` <Tag intent="warning">Deprecated</Tag>
- `@coinbase/cds-mobile-visualization@4.0.0` <Tag intent="warning">Deprecated</Tag>

The visualization packages will continue to receive updates until they are removed in CDS v10. New code should import from `@coinbase/cds-web/visualizations/*` and `@coinbase/cds-mobile/visualizations/*` (see [Visualization Packages](#visualization-package-consolidation)).
...
Look for the <Tag intent="promotional" colorScheme="blue">Migration Script ✓</Tag> tag throughout this guide to identify changes that are handled automatically.
...
See the [migrator README](https://github.com/coinbase/cds-staging/blob/master/packages/migrator/README.md) for the full CLI reference.
...
### 🗑️ Dropped (Removed) Deprecations
...
2. Migrate each call site to the modern v8 API by following the [v8 Migration Guide](./v8-migration-guide).
...
Yes. `@coinbase/cds-migrator` ships with a `v8-to-v9` preset covering button variants, `useMergeRefs`, layout types, and visualization imports. See the [Migration Script](#migration-script) section above and the [migrator README](https://github.com/coinbase/cds-staging/blob/master/packages/migrator/README.md) for the full CLI reference.
