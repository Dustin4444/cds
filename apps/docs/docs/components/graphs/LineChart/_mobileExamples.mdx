  ...
  const Example = memo(() => {
    ...
    return (
      ...
      >
        ...
        <Scrubber hideLine={!isScrubbing} hideOverlay />
      </CartesianChart>
    );
  });
...
You can use `gradient` with dynamic stops to highlight specific segments of a line based on scrubber position.
...
You can show sampled data at rest for performance and switch to full-resolution data when the user begins scrubbing, providing an adaptive level of detail.
...
function AdaptiveDetail() {
  ...
  const chartTransition = useMemo(() => ({ duration: 150 }), []);
  const chartYAxis = useMemo(
    () => ({
      range: ({ min, max }) => ({ min: min + 8, max: max - 8 }),
    }),
    [],
  );

  const MemoizedChart = memo(
    ({ highlight, data, isScrubbing, onHighlightChange, scrubberLabel }) => {
      return (
        <LineChart
          enableHighlighting
          height={200}
  ...
  const AdaptiveDetailChart = memo(() => {
    ...
    const formatDate = useCallback((date, periodId) => {
      const time = date.toLocaleTimeString('en-US', {
        hour: 'numeric',
        minute: '2-digit',
        hour12: true,
