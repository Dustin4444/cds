  ...
  const AssetPriceChart = memo(() => {
    ...
    const currentTimePrice = useMemo(() => {
      if (highlightedIndex !== undefined) {
        return data[highlightedIndex].value;
      }
      return currentPrice;
    }, [data, highlightedIndex, currentPrice]);
    ...
    const scrubberLabel = useMemo(() => {
      ...
      return formatDate(data[highlightedIndex].date);
    }, [highlightedIndex, data, formatDate]);
    ...
    const accessibilityLabel = useMemo(() => {
      ...
      }).format(data[highlightedIndex].value);
      const date = formatDate(data[highlightedIndex].date);
