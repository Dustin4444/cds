...
CartesianChart has built-in highlighting functionality that can be enabled with the `enableHighlighting` prop. This will then enable the usage of `onHighlightChange` to get the current highlighted position as the user interacts with the chart.

One example of using highlighting is to provide haptic feedback when the user interacts with the chart. You can trigger a light impact each time the highlighted position changes or even do a dynamic impact depending on the value change, such as a heavy impact when the user crosses a significant boundary of time or reaches a significant market event.
...
function Highlighting() {
  ...
  const handleHighlightChange = useCallback(
    (items) => {
      ...
      // Do a light impact when the highlighted position changes
      // An initial and final impact is already configured by the chart
      if (highlightedIndex !== undefined && index !== undefined) {
        void Haptics.lightImpact();
      }
      setHighlightedIndex(index ?? undefined);
    },
    [highlightedIndex],
  );
  ...
  return (
    <VStack gap={2}>
      <Text font="label1">Highlighted index: {highlightedIndex ?? 'none'}</Text>
      <LineChart
        enableHighlighting
...
function PriceWithVolume() {
  ...
  const handleHighlightChange = useCallback((items) => {
    setHighlightedIndex(items[0]?.dataIndex ?? null);
  }, []);

  const displayIndex = highlightedIndex ?? btcPrices.length - 1;
  ...
  const accessibilityLabel = useMemo(() => {
    if (highlightedIndex === null) return `Current Bitcoin price: ${formatPrice(currentPrice)}, Volume: ${formatVolume(currentVolume)}`;
    return `Bitcoin price at ${formatDate(currentDate)}: ${formatPrice(currentPrice)}, Volume: ${formatVolume(currentVolume)}`;
  }, [highlightedIndex, currentPrice, currentVolume, currentDate, formatPrice, formatVolume, formatDate]);
