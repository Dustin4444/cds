...
Enable automatic page transitions with the `autoplay` prop. The carousel will automatically advance to the next page at regular intervals.

### Basic Autoplay

When `autoplay` is enabled, a play/pause button appears in the navigation controls.
...
Use the `autoplayInterval` prop to customize the time between transitions (in milliseconds). The default is 3000ms (3 seconds).
...
Combine `autoplay` with `loop` for an infinite auto-scrolling carousel.

```jsx live
function AutoplayLoopCarousel() {
  function SquareAssetCard({ imageUrl, name }) {
    return (
      <ContainedAssetCard
        description={
...
:::tip Pause Preserves Progress

When the user pauses the autoplay, the remaining time is preserved. For example, if 2 seconds have elapsed out of a 3 second interval, pressing play will resume with only 1 second remaining before the next transition.

:::

### Pause on Interaction

By default, autoplay pauses when the user hovers anywhere over the carousel area, including the title, navigation, content, and pagination (`autoplayPauseOnInteraction` defaults to `true`). You can customize the delay before autoplay resumes after the user stops interacting using the `autoplayResumeDelay` prop.

```jsx live
function PauseOnInteractionCarousel() {
  function SquareAssetCard({ imageUrl, name }) {
    return (
      <ContainedAssetCard
        description={
  ...
  return (
    <Box marginX={-3}>
      <Carousel
        autoplay
        autoplayResumeDelay={500}
        loop
        title="Hover to pause (resumes after 500ms)"
        styles={{
          root: { paddingInline: 'var(--space-3)' },
          carousel: { gap: 'var(--space-1)' },
        }}
...
You can disable this behavior by setting `autoplayPauseOnInteraction` to `false`:

```jsx live
function NoPauseOnInteractionCarousel() {
  function SquareAssetCard({ imageUrl, name }) {
    return (
      <ContainedAssetCard
        description={
  ...
  return (
    <Box marginX={-3}>
      <Carousel
        ...
        loop
        title="Continues playing on hover"
        styles={{
          root: { paddingInline: 'var(--space-3)' },
          carousel: { gap: 'var(--space-1)' },
        }}
