...
You can render a two-line scrubber label by returning a newline-delimited string from `label`.
To keep the label positioned correctly, use the same alignment state in both places:

- `setAlignment('left' | 'center' | 'right')`
- `<DefaultScrubberLabel {...props} horizontalAlignment="center" />`
- Set each line's `textAnchor` and `x` from the active alignment.
...
function TwoLineScrubberLabel() {
  ...
  const AlignedScrubberLabel = memo((props) => {
    const [priceLabel, dayLabel = ''] = String(props.children ?? '').split('\n');
    const textLayout = useMemo(() => {
      const centerLayout = { anchor: 'middle', x: props.x };
      ...
      const resolveThemeFontValue = (value, fallback) => {
      ...
      const measureText = ({ fontSize, fontWeight, fontFamily }, label) => {
      ...
      if (alignment === 'left') {
        return { anchor: 'start', x: props.x - maxLineWidth / 2 };
      }

      return { anchor: 'end', x: props.x + maxLineWidth / 2 };
