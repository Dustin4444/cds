...
You can render a two-line scrubber label by returning a Skia paragraph from `label`.
To keep label positioning correct, use the same alignment value in both places:

- `Skia.ParagraphBuilder.Make({ textAlign: alignment }, fontMgr)`
- `<DefaultScrubberLabel {...props} paragraphAlignment={alignment} />`

```tsx
// Shared setup (same for Left, Center, and Right)
...
const scrubberLabel = useCallback(
  (index: number) => {
    ...
    const builder = Skia.ParagraphBuilder.Make({ textAlign: alignment }, fontMgr);
  ...
  [alignment, data, formatPrice, fontMgr, priceStyle, dayStyle],
);
...
<Scrubber labelElevated LabelComponent={AlignedScrubberLabel} label={scrubberLabel} />;
```

Only this value changes for each alignment:
