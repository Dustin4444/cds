...
function PositiveAndNegativeCashFlow() {
  ...
  const series = [
    { id: 'gains', label: 'Gains', data: gains, color: 'var(--color-fgPositive)' },
    { id: 'losses', label: 'Losses', data: losses, color: 'var(--color-fgNegative)' },
  ];
  ...
  return (
    ...
    >
      <ChartTooltip
        label={(dataIndex) => `March ${dataIndex + 1}`}
    ...
    >
      <ChartTooltip
        valueFormatter={(value) => {
          ...
          return `${tickFormatter(value[0])} - ${tickFormatter(value[1])}`;
        }}
      />
    </BarChart>
  );
