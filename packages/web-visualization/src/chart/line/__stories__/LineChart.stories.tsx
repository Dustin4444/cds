...
function HighlightLineSegments() {
  ...
  const handleHighlightChange = useCallback((dataIndex?: number) => {
...
function AdaptiveDetail() {
  ...
  // Returns both sampled values and corresponding sampled timestamps
  const sampleData = useCallback(
    (
      ...
      for (let i = 0; i < targetPoints; i++) {
        ...
        sampledValues.push(smoothed[idx]);
        sampledTimestamps.push(timestamps[idx]);
      }

      // Always include the last point for accuracy
      sampledValues[sampledValues.length - 1] = data[data.length - 1];
  ...
  type MemoizedChartProps = {
    ...
    timestamps: number[];
    isScrubbing: boolean;
    onScrubberPositionChange: (dataIndex?: number) => void;
  ...
  const MemoizedChart = memo(
    ({
      ...
      // Debug logging for scrubber position changes
      const handlePositionChange = useCallback(
        (dataIndex?: number) => {
          console.log('[AdaptiveDetail] scrubber position change:', {
            dataIndex,
        ...
        [data.length, timestamps.length, isScrubbing, onScrubberPositionChange],
      );
  ...
  const AdaptiveDetailChart = memo(() => {
    ...
    // Convert timestamps to numbers for xAxis
    const sparklineTimePeriodDataTimestampsNumeric = useMemo(() => {
      return sparklineTimePeriodDataTimestamps.map((d) => d.getTime());
    }, [sparklineTimePeriodDataTimestamps]);
    ...
    const sampledData = useMemo(() => {
      return sampleData(
        sparklineTimePeriodDataValues,
        sparklineTimePeriodDataTimestampsNumeric,
        samplePointCount,
      );
    }, [sparklineTimePeriodDataValues, sparklineTimePeriodDataTimestampsNumeric, samplePointCount]);

    // Use sampled data for display when idle, full data when scrubbing
    const displayData = useMemo(() => {
      return isScrubbing ? sparklineTimePeriodDataValues : sampledData.values;
    }, [isScrubbing, sparklineTimePeriodDataValues, sampledData.values]);

    const displayTimestamps = useMemo(() => {
      return isScrubbing ? sparklineTimePeriodDataTimestampsNumeric : sampledData.timestamps;
    }, [isScrubbing, sparklineTimePeriodDataTimestampsNumeric, sampledData.timestamps]);
    ...
    const scrubberLabel = useCallback(
      (index: number) => {
        // With xAxis timestamps, we can directly use the timestamp at the given index
        // The chart will interpolate to the correct position based on timestamp values
        const timestamp = displayTimestamps[index];
        if (timestamp === undefined) {
          return '';
        }
        return formatDate(new Date(timestamp), timePeriod.id);
      },
